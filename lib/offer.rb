...
class Offer
  ...
  def initialize(period_start, period_end, items_threshold, paid_items)
    ...
    @items_threshold, @paid_items = items_threshold, paid_items
  end
  ...
  def calculate_price (quantity, single_unit_price, options = {})
    options = {
        :currency => ''
    }.merge(options)
    ...
    AuditSingleton.instance.log  "Price: Reduced items #{quantity - non_reduced_items} (#{reduced_items})"\
                                  "* #{single_unit_price} #{currency}"\
                                  "= #{reduced_price_items} #{currency} "
    AuditSingleton.instance.log  "Price: Non reduced items #{non_reduced_items}"\
                                  "* #{single_unit_price} #{currency}"\
                                  "= #{normal_price_items} #{currency}"
    reduced_price_items + normal_price_items
  end

  def to_s
    "Period: #{@period_start} - #{@period_end}. #{@items_threshold} for #{@paid_items}"
  end
